...
| **Responsive navigation** — off-canvas drawer on phones, static sidebar from `xl` up, with no component library | `templates/partials/sidebar.html`, `topbar.html`, `www/js/ui.js` |
...
To change the styling, download the [Tailwind standalone CLI](https://github.com/tailwindlabs/tailwindcss/releases)
— **v4.3.3** is what the committed stylesheet was built with, and no Node.js is needed — put it in
`tools/`, then regenerate from the demo folder:

```bash
tools/tailwindcss -i src/input.css -o www/css/output.css --config tools/tailwind.config.js --minify
```

Use `--watch` instead of `--minify` while developing. The CLI itself is not committed: it is a
~107 MB binary, above GitHub's per-file limit.

`tools/tailwind.config.js` lists **every** file Tailwind has to scan — the templates, but also
`www/js/ui.js` and `Server.Web.Models.pas`, because both name utility classes that appear nowhere in
the HTML. A class Tailwind cannot see is a class it does not emit. The tutorial explains how to wire
the rebuild into a RAD Studio post-build event.

## Third-party assets

- **Tailwind CSS** — MIT. Only the compiled output (`www/css/output.css`) is bundled.
- **htmx** (`www/js/htmx.min.js`) — 0BSD licence, bundled.
- **DelphiZXIngQRCode** (`ThirdParty/`) — QR-code generation; see `ThirdParty/LICENSE.md`.
- **No component library.** The drawer and the account menu are handled by `www/js/ui.js`, written
  for this demo. The usual off-the-shelf choice, **Tailwind Plus Elements**, is commercially
  licensed — as is the Tailwind Plus UI Blocks markup it pairs with — so neither is used or
  redistributed here. The tutorial's *Interactive components: two options* section documents both
  routes, including what changes if you do hold a Tailwind Plus licence.
